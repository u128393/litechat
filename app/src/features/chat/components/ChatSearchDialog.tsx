...
export function ChatSearchDialog({ open, onOpenChange }: ChatSearchDialogProps) {
  ...
  const searchingDotCount = isSearching ? searchingDots : 3;
  ...
  const clampedActiveIndex = Math.min(activeIndex, Math.max(listItems.length - 1, 0));
  const activeItem = listItems[clampedActiveIndex];
  ...
  useEffect(() => {
    if (!open) {
      ...
      clearKeyboardScrollIndicator({ updateState: false });
      return;
    }
  ...
  useLayoutEffect(() => {
    ...
    if (scrollItemIntoView(clampedActiveIndex)) {
      showKeyboardScrollIndicator();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeItem?.id, clampedActiveIndex, listItems.length, open]);
  ...
  function handleQueryChange(nextQuery: string) {
    const nextIsQuerying = nextQuery.trim() !== "";
    setQuery(nextQuery);
    setActiveIndex(0);
    setResults([]);
    setIsSearching(nextIsQuerying);
    setHasSearched(false);
    setSearchingDots(3);
  }
  ...
  function clearKeyboardScrollIndicator({ updateState = true }: { updateState?: boolean } = {}) {
    if (keyboardScrollTimerRef.current !== null) {
      window.clearTimeout(keyboardScrollTimerRef.current);
      keyboardScrollTimerRef.current = null;
    }
  ...
  function moveActiveIndex(delta: number) {
    const nextIndex = Math.min(Math.max(clampedActiveIndex + delta, 0), Math.max(listItems.length - 1, 0));
                    ...
                    <div key={group.label} className="flex flex-col gap-1">
                      ...
                      {group.items.map(({ item, index }) => (
                        <SearchItemRow
                          key={item.id}
                          item={item}
                          selected={index === clampedActiveIndex}
                ...
                <div className="flex flex-col gap-1">
                  {searchItems.map((item, index) => (
                    <SearchItemRow
                      ...
                      selected={index === clampedActiveIndex}
                      resultMode
                      showDate={index === clampedActiveIndex}
                      onPointerHover={(event) => handlePointerHover(item.id, index, event)}
                      onPointerLeave={() => handlePointerLeave(item.id)}
                      onClick={() => {
                        void activateItem(item);
